...
def build_ngram_index(tokenized_documents, ngrams):
    ...
    index = dict()
    for doc_id in tokenized_documents:
        grams = list(nltk.bigrams(tokenized_documents[doc_id])) + list(nltk.trigrams(tokenized_documents[doc_id]))

        for gram in ngrams:
            # Calculating gram freq
            if gram in grams:
                if gram in index:
                    gram_freq, doc_dict = index[gram]
                    ...
                    index[gram] = (gram_freq, doc_dict)
                else:
                    doc_dict = {doc_id: 1}
                    index[gram] = (1, doc_dict)

    for gram in index:
        gram_freq, doc_dict = index[gram]
        index[gram] = [gram_freq] + list(doc_dict.items())
