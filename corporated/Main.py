...
def okapi_scoring(query, doc_lengths, index, k1=1.2, b=0.75):
    ...
    :return: dictionary of scores - doc_id:score
    """
    scores = dict([(doc, 0) for doc in doc_lengths.keys()])
    idf_dict_vector = compute_idf_vector(len(doc_lengths), index)
    avgdl = sum(doc_lengths.values()) / len(doc_lengths)

    for q, tq_f in query.items():
        if q not in index:
            continue

        idf = idf_dict_vector[q]

        for doc, dtf in index[q][1:]:
            scores[doc] += idf * dtf * (k1 + 1) / (dtf + k1 * (1 - b + b * doc_lengths[doc] / avgdl))

    return scores


def answer_query(raw_query, index, doc_lengths, documents,
                 top_k, scoring_fnc, k_gram_index=None, expansion=None):
    """
    :param tol_retrieval:
    :param k_gram_index:
    ...
    """
    # If spelling correction enabled
    if k_gram_index is not None:
        words = tokenize(raw_query)
        for i, word in enumerate(words):
            if word not in index:
                options = spell_checker.generate_wildcard_options(word, k_gram_index)
                # Choosing random option
                words[i] = options[random.randint(0, len(options) - 1)]
        raw_query = " ".join(words)

    # Perform query expansion using Wordnet if enabled
    if expansion is not None:
        print("Old query: " + raw_query)
        raw_query = query_expansion.get_wordnet_query(raw_query)
    ...
    for k in range(top_k):
        ...
        article = document_summarization.LSA_summary(article)

        article_terms = tokenize("".join(article))
...
def answer_query_inexact(raw_query, index, doc_lengths, documents,
                         top_k, scoring_fnc, k_gram_index=None, expansion=None):
    """
    :param tol_retrieval:
    :param k_gram_index:
    ...
    query = Counter(query)
    # Retrieve doc_ids
    doc_ids = high_low.filter_docs(query, index, top_k)
    # retrieve scores for documents given doc_ids
    scores = scoring_fnc(query, doc_ids, doc_lengths, index)
    # put them in heapq data structure, to allow convenient extraction of top k elements
    h = []
    for doc_id in scores.keys():
        neg_score = -scores[doc_id]
...
def main():
    ...
    obj_path = 'obj/'
    if not os.path.isfile(obj_path + 'reuters_index.pkl'):
        build_index(reuters_path)
    with open(obj_path + 'reuters_index.pkl', 'rb') as fp:
        print('reuters_index.pkl: reading done')
        index = pickle.load(fp)
    with open(obj_path + 'reuters_doc_length.pkl', 'rb') as fp:
        print('reuters_doc_lengths.pkl: reading done')
        doc_lengths = pickle.load(fp)
    with open(obj_path + 'reuters_documents.pkl', 'rb') as fp:
        print('reuters_documents.pkl: reading done')
        documents = pickle.load(fp)

    if not os.path.isfile(obj_path + 'k_gram_index.pkl'):
        # Making dictionary with words
        word_set = set()
        for key in documents:
            words = tokenize(documents[key])
            words = [word.lower() for word in words if word.isalpha()]
            word_set.update(set(words))

        spell_checker.build_k_gram_index(word_set, 3)
    with open(obj_path + 'k_gram_index.pkl', 'rb') as fp:
        print('k_gram_index.pkl: reading done')
        k_gram_index = pickle.load(fp)

    if not os.path.isfile(obj_path + 'high_low_index.pkl'):
        high_low.build_high_low_index(index, 4)
    with open(obj_path + 'high_low_index.pkl', 'rb') as fp:
        print('high_low_index.pkl: reading done')
        high_low_index = pickle.load(fp)

    if not os.path.isfile(obj_path + 'categories.pkl'):
        lang_model.extract_categories(reuters_path)
    with open(obj_path + 'categories.pkl', 'rb') as fp:
        print('categories.pkl: reading done')
        categories = pickle.load(fp)

    # Basic scoring using Inverted Index and Document summarization
    answer_query("apple personal", index, doc_lengths, documents, 3, cosine_scoring)
    answer_query("apple personal", index, doc_lengths, documents, 3, okapi_scoring)

    # Tolerant retrieval based on k-gram index
    answer_query("ap*le personal", index, doc_lengths, documents, 3, cosine_scoring, k_gram_index)
    answer_query("ap*le personal", index, doc_lengths, documents, 3, okapi_scoring, k_gram_index)

    # Inexact retrieval. Trying to see performance boost
    # start = time.time()
    # answer_query_inexact("apple personal", high_low_index, doc_lengths, documents, 3, high_low.cosine_scoring_docs)
    # end = time.time()
    # inex_time = end - start
    #
    # start = time.time()
    # answer_query("apple personal", index, doc_lengths, documents, 3, cosine_scoring)
    # end = time.time()
    # print("Standard retrieval time: " + str(end - start))
    # print("Inexact retrieval time: " + str(inex_time))

    # Language modeling
    #answer_query_inexact("apple personal", high_low_index, doc_lengths, documents, 3, lang_model.lm_rank_documents)

    # Query expansion using Wordnet. VERY INTERESTING VARIANTS
    # answer_query("apple personal", index, doc_lengths, documents, 3, cosine_scoring, expansion="wordnet")
    # answer_query_inexact("apple personal", high_low_index, doc_lengths,
    #                      documents, 3, lang_model.lm_rank_documents, expansion="wordnet")
